...
void __am_gpu_init() {
  ...
  uint32_t *fb = (uint32_t *)(uintptr_t)FB_ADDR; // map gpu memory
  memset(fb, 0, w * h * sizeof(uint32_t)); // clean screen
  outl(SYNC_ADDR, 1);
}
...
void __am_gpu_fbdraw(AM_GPU_FBDRAW_T *ctl) {
  ...
  uint32_t *dst = fb + screen_w * y + x; // dst 指向显存
  uint32_t *src = pixels;
  for (int i = 0; i < h; i++) { // 逐行复制
